...
public class CustomerOrder {
	...
	public double getTotal(){
		...
		// Add labor cost
        subTotal += this.laborPrice;

        // Add part cost
        subTotal += this.partPrice;

		
		// apply discount based on the selected discount rate
	    double discountRate = this.discount;
	    double total = subTotal;
	    if (discountRate == 0.10) {
	        total -= total * 0.10; // apply 10% discount
	    } else if (discountRate == 0.20) {
	        total -= total * 0.20; // apply 20% discount
	    }
